...
const MovieHeader: FC<{ movie: Movie, movieActors: any, movieDirectors: any, movieWriters: any }> = ({ movie, movieActors, movieDirectors, movieWriters }) => {
    ...
    <div className="bg-[#262a29] p-4 sm:px-32 md:px-28 lg:px-40">
      ...
      <div className="flex gap-2 py-2 px-1">
        ...
        <span className="text-blue-500 text-sm">
          {movieDirectors.map((directors: any) => {
            return <span>{directors.director.directorName} • </span>;
          })}
        </span>
      </div>
      <hr className="border-gray-600" />
      <div className="flex gap-2 py-2 px-1">
        ...
        <span className="text-blue-500 text-sm">
          {movieWriters.map((writers: any) => {
            return <span>{writers.writer.writerName} • </span>;
          })}
        </span>
      </div>
      <hr className="border-gray-600" />
      <div className="flex gap-2 py-2 px-1">
        ...
        <span className="text-blue-500 text-sm">
          {movieActors.map((actors: any) => {
            return <span>{actors.actor.actorName} • </span>;
          })}
        </span>
      </div>
      <hr className="border-gray-600" />
